...
class Identity(_BaseMetric):
    ...
    def plot_single_tracker_idf1_results(self, table_res, tracker, cls, output_folder):
        """Plot IDF1 (CLR_F1) as horizontal line across alpha range"""
        import matplotlib.pyplot as plt
        import numpy as np
        import os

        res = table_res['COMBINED_SEQ']
        idf1 = res.get('CLR_F1', 0.0)

        alpha = np.linspace(0, 1, 21)
        idf1_line = [idf1] * len(alpha)

        plt.plot(alpha, idf1_line, 'g', label=f'IDF1 ({idf1:.2f})')
        plt.xlabel('alpha')
        plt.ylabel('score')
        ...
        out_file = os.path.join(output_folder, cls + '_IDF1_plot.pdf')
