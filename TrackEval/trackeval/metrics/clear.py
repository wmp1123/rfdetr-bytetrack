...
class CLEAR(_BaseMetric):
    ...
    def __init__(self, config=None):
        ...
        # self.float_array_fields = ['MOTA', 'CLR_FN', 'CLR_FP', 'IDSW']
    ...
    def plot_single_tracker_mota_results(self, table_res, tracker, cls, output_folder):
        """Plot MOTA as a horizontal line across alpha range (styled like HOTA plot)"""
        import matplotlib.pyplot as plt
        import numpy as np
        import os

        res = table_res['COMBINED_SEQ']
        mota = res.get('MOTA', 0.0)

        alpha = np.linspace(0, 1, 21)
        mota_line = [mota] * len(alpha)

        plt.plot(alpha, mota_line, 'r', label=f'MOTA ({mota:.2f})')
        plt.xlabel('alpha')
        plt.ylabel('score')
        plt.title(f'{tracker} - {cls}')
        plt.axis([0, 1, 0, 1])
        plt.legend(loc='lower left')

        out_file = os.path.join(output_folder, cls + '_MOTA_plot.pdf')
