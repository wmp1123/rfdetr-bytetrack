                            ...
                            for metric, metric_name in zip(metrics_list, metric_names):
                                ...
                                if config['PLOT_CURVES']:
                                    metric.plot_single_tracker_results(table_res, tracker_display_name, c_cls,
                                                                       output_fol)
                                    # Additional plots for scalar metrics (like MOTA, IDF1)
                                    if hasattr(metric, 'plot_single_tracker_mota_results'):
                                        metric.plot_single_tracker_mota_results(table_res, tracker_display_name, c_cls, output_fol)
                                    if hasattr(metric, 'plot_single_tracker_idf1_results'):
                                        metric.plot_single_tracker_idf1_results(table_res, tracker_display_name, c_cls, output_fol)
                            if config['OUTPUT_SUMMARY']:
